...
(h=(0!==g.attr("data-image-description").length?"<br />":"")+c("<textarea />").html(b.download).val(),g.attr("data-image-description",g.attr("data-image-description")+h)));g.attr("src","data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw%3D%3D");f.append(g);b.linkContainer=f;void 0!==a.currentHiddenLink?c(a.currentHiddenLink).after(f):a.element.find(".jig-clearfix").after(f);a.currentHiddenLink=f},ba=function(a,d,h){d.off("load");0!==a.length&&a.off("load").remove();d.clone().addClass("jig-pixastic").insertAfter(d).on("load",
